...
export default function Search(props)
{
   ...
   {
      if (stringSimilarity.compareTwoStrings(props.movies[i].title, text) >= 0.5 || stringSimilarity.compareTwoStrings(props.movies[i].author, text) >= 0.5)
      {
         similar.push(props.movies[i])
         continue
      }
   }
   return (
      <div className={classes.root}>
         ...
         <div className={classes.header_holder}><Header className={classes.header_page}>Search results for '{text}' - {similar.length} results.</Header></div>
